...
if [[ $TRAVIS_SCALA_VERSION = "ref-211" ]]; then
  sbt '++ 2.11.3' 'benchmark RefBenchmark'
elif [[ $TRAVIS_SCALA_VERSION = "int-211" ]]; then
  sbt '++ 2.11.3' 'benchmark IntBenchmark'
  ...
  sbt '++ 2.11.3' test $(if [[ "${TRAVIS_PULL_REQUEST}" == "false" && "${TRAVIS_BRANCH}" == "master" ]]; then echo publish ; fi)
