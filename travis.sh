...
if [[ $TRAVIS_SCALA_VERSION = "ref-211" ]]; then
  sbt -Ddisable_sxr '++ 2.11.4' 'benchmark RefBenchmark'
elif [[ $TRAVIS_SCALA_VERSION = "int-211" ]]; then
  sbt -Ddisable_sxr '++ 2.11.4' 'benchmark IntBenchmark'
  ...
  sbt '++ 2.11.4' test $(if [[ "${TRAVIS_PULL_REQUEST}" == "false" && "${TRAVIS_BRANCH}" == "master" ]]; then echo publish ; fi)
